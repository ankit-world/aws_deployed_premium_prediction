...
class Raw_Data_validation:
    ...
    def validateColumnLength(self, NumberofColumns):
        ...
        try:
            ...
            for file_ in os.listdir(os.path.join("Training_Raw_files_validated/", "Good_Raw/")):
                df = pd.read_csv(os.path.join("Training_Raw_files_validated/", "Good_Raw/") + file_)
    ...
    def validateMissingValuesInWholeColumn(self):
        ...
        try:
            ...
            for file_ in os.listdir(os.path.join("Training_Raw_files_validated/", "Good_Raw/")):
                csv = pd.read_csv(os.path.join("Training_Raw_files_validated/", "Good_Raw/") + file_)
                count = 0
                for columns in csv:
                    if (len(csv[columns]) - csv[columns].count()) == len(csv[columns]):
                        ...
                        break
                if count == 0:
                    csv.rename(columns={"Unnamed: 0": "Insurance"}, inplace=True)
                    csv.to_csv(os.path.join("Training_Raw_files_validated/", "Good_Raw/") + file_, index=None,
                               header=True)
